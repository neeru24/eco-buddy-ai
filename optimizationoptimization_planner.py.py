...
class OptimizationPlanner:
    ...
    def __init__(self):
        ...
        self.phase_estimates = self._initialize_phase_estimates()
    ...
    def _initialize_plan_templates(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            'quick_wins': {
                ...
                'description': 'Focus on low-effort, high-impact improvements',
                'duration_weeks': 4,
                'focus_areas': ['energy', 'water', 'waste']
            },
            'balanced': {
                'name': 'Balanced Optimization Plan',
                'description': 'Balanced approach across all categories',
                'duration_weeks': 12,
                'focus_areas': ['energy', 'water', 'waste', 'food', 'transportation', 'shopping']
            },
            'deep_optimization': {
                'name': 'Deep Optimization Plan',
                'description': 'Comprehensive optimization with major upgrades',
                'duration_weeks': 24,
                'focus_areas': ['energy', 'water', 'waste', 'food', 'transportation', 'shopping']
            }
        }
    
    def _initialize_phase_estimates(self) -> Dict[str, Dict[str, Any]]:
        """
        Initialize phase estimates.
        """
        return {
            'phase1': {
                'name': 'Foundation',
                'duration_weeks': 4,
                'description': 'Quick wins and basic improvements'
            },
            'phase2': {
                'name': 'Optimization',
                'duration_weeks': 8,
                'description': 'Systematic improvements across categories'
            },
            'phase3': {
                'name': 'Advanced',
                'duration_weeks': 12,
                'description': 'Major upgrades and deep optimization'
            }
        }
    ...
    def create_plan_from_template(self,
                                 household_id: str,
                                 template_key: str,
                                 custom_targets: List[Dict[str, Any]] = None) -> OptimizationPlan:
        """
        ...
        Returns:
            OptimizationPlan: Created plan
        """
        template = self.plan_templates.get(template_key, self.plan_templates['balanced'])
        ...
        # Add default targets based on template
        if custom_targets:
            ...
            for area in template['focus_areas']:
                target = OptimizationTarget(
                    plan_id=plan.id,
                    category=self._get_category_for_area(area),
                    target_value=self._get_default_target(area),
                    unit=self._get_unit_for_area(area),
                    deadline=datetime.now() + timedelta(days=template['duration_weeks'] * 7)
                )
                plan.targets.append(target)
        
        return plan
    
    def _get_category_for_area(self, area: str) -> OptimizationCategory:
        """
        Get category for focus area.
        """
        mapping = {
            'energy': OptimizationCategory.ENERGY_EFFICIENCY,
            'water': OptimizationCategory.WATER_CONSERVATION,
            'waste': OptimizationCategory.WASTE_REDUCTION,
            'food': OptimizationCategory.FOOD_OPTIMIZATION,
            'transportation': OptimizationCategory.TRANSPORTATION_OPTIMIZATION,
            'shopping': OptimizationCategory.SHOPPING_OPTIMIZATION
        }
        return mapping.get(area, OptimizationCategory.OTHER)
    
    def _get_default_target(self, area: str) -> float:
        """
        Get default target for focus area.
        """
        mapping = {
            'energy': 20.0,
            'water': 30.0,
        ...
        return mapping.get(area, 10.0)
    
    def _get_unit_for_area(self, area: str) -> str:
        """
        Get unit for focus area.
        """
        mapping = {
            'energy': '%',
            'water': '%',
    ...
    def update_target_progress(self, 
        ...
        target.progress_history.append({
    ...
    def calculate_plan_progress(self, plan: OptimizationPlan) -> Dict[str, Any]:
        ...
        for target in plan.targets:
            ...
            if cat not in category_progress:
                category_progress[cat] = {'completed': 0, 'total': 0}
        ...
        for cat in category_progress:
            total = category_progress[cat]['total']
            completed = category_progress[cat]['completed']
            category_progress[cat]['percentage'] = (completed / total * 100) if total > 0 else 0
    ...
    def generate_progress_report(self, 
        ...
        return {
            ...
            'category_progress': progress.get('category_progress', {}),
            ...
            'actions_completed': plan.completed_actions,
    ...
    def prioritize_actions(self, 
        ...
        for target in plan.targets:
            ...
            actions.append({
                ...
                'effort': self._get_effort_level(target)
            })
    ...
    def _get_recommended_action(self, target: OptimizationTarget) -> str:
        ...
        actions = {
            ...
            OptimizationCategory.SHOPPING_OPTIMIZATION: "Adopt sustainable shopping habits",
    ...
    def _get_effort_level(self, target: OptimizationTarget) -> str:
        """
        Get effort level for a target.
        """
        effort_levels = {
            OptimizationCategory.ENERGY_EFFICIENCY: "medium",
            OptimizationCategory.WATER_CONSERVATION: "medium",
            OptimizationCategory.WASTE_REDUCTION: "low",
            OptimizationCategory.FOOD_OPTIMIZATION: "medium",
            OptimizationCategory.TRANSPORTATION_OPTIMIZATION: "medium",
            OptimizationCategory.SHOPPING_OPTIMIZATION: "low",
            OptimizationCategory.BEHAVIORAL_CHANGE: "low",
            OptimizationCategory.TECHNOLOGY_UPGRADE: "high"
        }
        return effort_levels.get(target.category, "medium")
            ...
            if target.deadline and not target.achieved:
                if target.deadline < now:
                    ...
                    missed_actions.append({
                        ...
                        'current_progress': (target.current_value / target.target_value * 100) if target.target_value > 0 else 0,
